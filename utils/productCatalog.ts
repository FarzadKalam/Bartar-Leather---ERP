...
export const LEGACY_VARIANT_FIELD_MAPPINGS: Record<string, string> = {
  ...
  leather_width: 'global_leather_width',
  ...
  lining_type: 'global_lining_type',
  lining_color: 'global_color',
  lining_width: 'global_lining_width',
  acc_material: 'global_accessory_material',
  accessory_type: 'global_accessory_type',
  accessory_width: 'global_accessory_width',
...
export const extractManualNamePrefix = (currentName: string, autoGeneratedName?: string | null) => {
  const normalizedCurrent = String(currentName || '').replace(/\s+/g, ' ').trim();
  const normalizedAuto = String(autoGeneratedName || '').replace(/\s+/g, ' ').trim();
  if (!normalizedCurrent) return '';
  if (!normalizedAuto) return normalizedCurrent;
  if (normalizedCurrent === normalizedAuto) return '';
  if (normalizedCurrent.endsWith(` ${normalizedAuto}`)) {
    return normalizedCurrent.slice(0, -(normalizedAuto.length + 1)).trim();
  }
  return normalizedCurrent;
};

export const composeNameWithAutoSuffix = (manualPrefix?: string | null, autoGeneratedName?: string | null) => {
  const normalizedPrefix = String(manualPrefix || '').replace(/\s+/g, ' ').trim();
  const normalizedAuto = String(autoGeneratedName || '').replace(/\s+/g, ' ').trim();
  if (!normalizedPrefix) return normalizedAuto;
  if (!normalizedAuto) return normalizedPrefix;
  if (normalizedPrefix === normalizedAuto || normalizedPrefix.endsWith(` ${normalizedAuto}`)) {
    return normalizedPrefix;
  }
  return `${normalizedPrefix} ${normalizedAuto}`;
};
